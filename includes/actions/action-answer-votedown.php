...
if ( ! defined('ABSPATH')) exit;  // if direct access 
	...
	if ( ! function_exists( 'qa_email_action_answer_votedown_function' ) ) {
		function qa_email_action_answer_votedown_function( $answer_ID ) {
			...
			$email_body = strtr($templates_data['answer_votedown']['html'], $vars);
			$email_subject =strtr($templates_data['answer_votedown']['subject'], $vars);
			...
			if( $qa_email_on_answer_voting == 'yes' ){
				...
				$class_qa_email_templates_design->qa_send_email( $author_data->user_email, $email_subject, $email_body );
